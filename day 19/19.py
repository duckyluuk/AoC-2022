            ...
            for state in states:
                ...
                for i in range(3,-1,-1):
                    ...
                    if all([resources[j]>=cost[j] for j in range(4)]):
                        ...
                        newStates.append(newState)
                        if i>1: break # if you can make a geode or obsidian bot then do so
            sortKey = lambda x: (x["resources"][3]+x["robots"][3]*(loops-i))*1000 + ( # sort all states
                                 x["resources"][2]+x["robots"][2]*(loops-i))*100  + ( # by what their final score would be
                                 x["resources"][1]+x["robots"][1]*(loops-i))*10   + (
                                 x["resources"][0]+x["robots"][0]*(loops-i)) 
            states = sorted(newStates, key = sortKey)[-250:]
